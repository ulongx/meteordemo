...
Meteor.methods({
  ...
  upvote: function(postId) {
    ...
    if (! affected)
      throw new Meteor.Error('无效', "你不能再投票了！");
    // var post = Posts.findOne(postId);
    // if (!post)
    //   throw new Meteor.Error('无效', '帖子未找到');
    // if (_.include(post.upvoters, this.userId))
    //   throw new Meteor.Error('无效', '你已投过票了');
    // /*
    // $addToSet 将一个 item 加入集合如果它不存在的话，
    // $inc 只是简单的增加一个整型属性。
    // */
